...
function Cart({ cart, setCart }) {
    ...
    useEffect(() => {
        // Desplazar la página hacia arriba cuando el componente se monte
        window.scrollTo(0, 0);
      }, []);

    // Calcular el subtotal del carrito
    const subtotal = cart.reduce((acc, item) => acc + parseFloat(item.precio.replace('$', '')), 0);
            ...
            <div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
                ...
                {cart.map(item => (
                    <div key={item.id} className="rounded-3xl border-2 border-gray-200 p-4 lg:p-8 grid grid-cols-12 mb-8 max-lg:max-w-lg max-lg:mx-auto gap-y-4">
                        <div className="col-span-12 lg:col-span-2 img box">
                            <img src={item.image} alt="" className="max-lg:w-full lg:w-[180px]" />
                        </div>
                        <div className="col-span-12 lg:col-span-10 detail w-full lg:pl-3">
                            <div className="flex items-center justify-between w-full mb-4">
                                <h5 className="font-manrope font-bold text-2xl leading-9 text-white">{item.title}</h5>
                                <button className="rounded-full group flex items-center justify-center focus-within:outline-red-500" onClick={() => removeFromCart(item.id)} style={{ marginTop: '10px' }}>
                            ...
                            <p className="font-normal text-base leading-7 text-white mb-2">Procesador: {item.procesador}</p>
                            <p className="font-normal text-base leading-7 text-white mb-6">Fecha de Lanzamiento: {item.fechaLanzamiento}</p>
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <h6 className="text-indigo-600 font-manrope font-bold text-2xl leading-9 text-right">{item.precio}</h6>
                            </div>
                        </div>
                    </div>
                ))}
                <div className="flex flex-col md:flex-row items-center md:items-center justify-between lg:px-6 pb-6 border-b border-gray-200 max-lg:max-w-lg max-lg:mx-auto">
                    <h5 className="text-white font-manrope font-semibold text-2xl leading-9 w-full max-md:text-center max-md:mb-4">Subtotal</h5>
                    <div className="flex items-center justify-between gap-5 ">
                        <h6 className="font-manrope font-bold text-3xl lead-10 text-indigo-600">${subtotal}</h6>
                    </div>
                </div>
                ...
                <div className="max-lg:max-w-lg max-lg:mx-auto">
                    ...
                    <div className="flex justify-center space-x-4 mb-6">
                        ...
                        <button onClick={clearCart} className="rounded-full py-4 px-6 bg-red-600 text-white font-semibold text-lg w-full text-center transition-all duration-500 hover:bg-red-700">Clear Cart</button>
                    </div>
                    <div className="text-center">
                        {/* Botón para regresar a la ruta '/' */}
                        <Link to="/" className="text-indigo-600 font-manrope font-bold text-lg">Back to Shop</Link>
                    </div>
                </div>
            </div>
            <Footer />
